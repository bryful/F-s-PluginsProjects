...
template <typename T>
static bool IsTargetColor(const T* p, const ParamInfo* info) {
...
static PF_Err UnifyToMarkerIterate(void* refcon, A_long x, A_long y, T* in, T* out) {
	ParamInfo* info = reinterpret_cast<ParamInfo*>(refcon);
...
/* 実行エンジン */
template <typename T>
static PF_Err ExecuteProcess(NF_AE* ae, ParamInfo* infoP) {
	...
	ERR(ae->NewWorld(ae->output->width,ae->output->height, ae->pixelFormat(), &temp));
	if (err) return err;

	// STEP 1: マーカー化
	if constexpr (std::is_same_v<T, PF_Pixel8>) err = ae->iterate8(ae->input, infoP, UnifyToMarkerIterate<T>,ae->output);
	else if constexpr (std::is_same_v<T, PF_Pixel16>) err = ae->iterate16(ae->input, infoP, UnifyToMarkerIterate<T>, ae->output);
	else err = ae->iterate32(ae->input, infoP, UnifyToMarkerIterate<T>, ae->output);
	...
	for (int i = 0; i < 100; i++) {
		...
		changed.store(0);
		if constexpr (std::is_same_v<T, PF_Pixel8>) {
			ERR(ae->suitesP->Iterate8Suite2()->iterate(ae->in_data, 0, ae->output->height, srcP, NULL, infoP, DarkestFillIterate<T>, dstP));
		}
		else if constexpr (std::is_same_v<T, PF_Pixel16>) {
			ERR(ae->suitesP->Iterate16Suite2()->iterate(ae->in_data, 0, ae->output->height, srcP, NULL, infoP, DarkestFillIterate<T>, dstP));
		}
		else {
			ERR(ae->suitesP->IterateFloatSuite2()->iterate(ae->in_data, 0, ae->output->height, srcP, NULL, infoP, DarkestFillIterate<T>, dstP));
		}
	...
	// STEP 3: 最終パス
	if constexpr (std::is_same_v<T, PF_Pixel8>) ae->iterate8(ae->input, infoP, FinalFillIterate<T>,ae->output);
	else if constexpr (std::is_same_v<T, PF_Pixel16>) ae->iterate16(ae->input, infoP, FinalFillIterate<T>, ae->output);
	else ae->iterate32(ae->input, infoP, FinalFillIterate<T>, ae->output);
