...
//----------------------------------------------------------------------------
PF_Boolean makeTbl(blurPrm *prm, blurPrmSub16 *sub)
{
	PF_InData *in_data;
	in_data = prm->in_data;
	...
	for ( y=0; y<sub->height; y++)
	{
		...
		for ( x=0; x<sub->width; x++){
			...
			if ( c.alpha==PF_MAX_CHAN16)
			{
				...
				if (count>0){
					c.red	= RoundShort(sr/count);                
					c.green = RoundShort(sg/count);                
					c.blue	= RoundShort(sb/count);
...
PF_Err BlurSub16(blurPrm *prm)
{
	...
	sub.scanline =*(PF_Pixel16**)prm->scanlineH; 
	sub.width = prm->width;
