...
//-------------------------------------------------------
inline A_long lineLen(PF_Fixed a,PF_Fixed b)
{
	double tx,ty,tl;
	tx = FIX2FLT(a);
	ty = FIX2FLT(b);
...
void drawTouchSub8(TouchDrawInfo *p)
{
	...
	//if (rndi>0) {
		...
		if (len>0) {
			...
			A_long i;
			for ( i=0; i<len;i++){
				if (PsetAlpha8(&(p->pp))==FALSE) break;
				//PsetAlphaB(&(p->pp));
				p->pp.pos.x += dx;
				p->pp.pos.y += dy;
				p->pp.opacity -= dl;
...
//-------------------------------------------------------
void drawTouch8(CFsGraph *g,ParamInfo *prm)
{	
	A_long w = g->width();
	A_long h = g->height();
	...
	//td.value = F_RAND_MAX * prm->target_value >> 16;
	//長さランダム
	PF_Fixed ir0,ir1;
	...
	for (j=0;j<h;j++){
		...
		for (i=0;i<w;i++){
			xx = i<<16;
			if (oD[targetOut].blue == PF_MAX_CHAN8){
				if ((A_long)F_RAND()<t0){
