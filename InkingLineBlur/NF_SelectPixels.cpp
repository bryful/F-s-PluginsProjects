	...
	T* outP)
{
	...
	else if (infoP->mode == SelectPixelMode::SP_MODE_COLOR)
	{
		*outP = infoP->custumColor;
	}
...
)
{
	...
	else if (mode > SelectPixelMode::SP_MODE_COLOR) mode = SelectPixelMode::SP_MODE_COLOR;
	...
	{
		SPInfo<PF_PixelFloat> info32;
		AEFX_CLR_STRUCT(info32);
		info32.mode = info8.mode;
		info32.count = info8.count;
		for (A_long i = 0; i < info8.count; i++) {
			info32.target_colors[i] = info8.target_colors[i];
		}
		info32.custumColor = NF_Pixel8To32(custum_color);
	...
	{
		SPInfo<PF_Pixel16> info16;
		AEFX_CLR_STRUCT(info16);
		info16.mode = info8.mode;
		info16.count = info8.count;
		for (A_long i = 0; i < info8.count; i++) {
			info16.target_colors[i] = info8.target_colors[i];
		}
		info16.custumColor = NF_Pixel8To16(custum_color);
	...
	{
		info8.custumColor = custum_color;
