...
//-------------------------------------------------------------------------------------------------
void BlockBevelSubTwo16(gridParam *prm)
{
	A_long x0,x1,y0,y1;
	...
	target2 = target;
	if ( (x0>=0)&&(y0>=0) ){
		data[target] = lineBright16(data[target],prm->info.hilight);
	}
	if ( (x1<prm->width)&&(y0>=0) ){
		target2 = target +1;
		data[target2] = lineBright16(data[target2],prm->info.hilight);
	}
	...
	if ( (x1<prm->width)&&(y1<prm->height) ){
		...
		data[target2] = lineBright16(data[target2],prm->info.shadow);
	}
}
//-------------------------------------------------------------------------------------------------
void BlockBevelSub16(gridParam *prm)
{
	A_long x0,x1,y0,y1;
	A_long target,i;
	A_long w,s0,s1;
	...
	if (y1<prm->height) {
		...
		for (i=0 ; i< w ;i++){
			data[target] = lineBright16(data[target],prm->info.shadow);
			target++;
		}
	}
...
void BlockMosaic16(gridParam *prm)
{
	...
	col.alpha	= RoundShort(a / count);
	col.blue	= RoundShort(b / count);
	col.green	= RoundShort(g / count);
	col.red		= RoundShort(r / count);
