...
#ifndef F_FIX2FLT
#define	F_FIX2FLT(X)	((double)(X) / 65536.0)
#endif
#ifndef F_FLT2FIX
#define	F_FLT2FIX(F)	((PF_Fixed)((F) * 65536 + (((F) < 0) ? -0.5 : 0.5)))
#endif
#ifndef F_ABS
...
// ------------------------------------------------------------------------------------------------ -
inline PF_PixelFloat NF_Pixel8ToFloat(PF_Pixel c)
{
	PF_PixelFloat r;
	r.alpha = (PF_FpShort)(NF_CONVERT8TO32(c.alpha));
	r.red = (PF_FpShort)(NF_CONVERT8TO32(c.red));
...
public:
	...
	double		rotFLT() { return F_FIX2FLT(m_rot);}
	double		lengthFLT() { return F_FIX2FLT(m_length);}
	double		xFLT() { return F_FIX2FLT(m_pos.x);}
	double		yFLT() { return F_FIX2FLT(m_pos.y);}
	//------------------------------
	...
	PF_FixedPoint SetRotLength(PF_Fixed r, PF_Fixed len)
	{
		...
		if (len != 0){
			...
			double r2 = F_FIX2FLT(rr);
			double v2 = F_FIX2FLT(len);
			double x = 0;
			double y = 0;
