...
{
	PF_Err		err 		= PF_Err_NONE;
	ERR(ae->GetFLOAT_float(ID_EVOLUTION, &infoP->evolution));
	infoP->evolution /= 100.0f;
	ERR(ae->GetADD(ID_OCTAVES, &infoP->octaves));
	ERR(ae->GetFLOAT_float(ID_SCALE, &infoP->scale));
	infoP->scale /= 100.0f;
