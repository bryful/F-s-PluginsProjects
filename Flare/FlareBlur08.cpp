...
//==================================================================================
static PF_Err BlurRedHor(CFsGraph *g, A_long blue, A_u_char *scanline, A_long *tbl)
{
	...
	for (A_long y = 0; y < h; y++)
	{
		...
		for (A_long x = 0; x <w; x++) {
			...
			if (count > 0) {
				data[adr + x].red = RoundByteLong(r / count);
			}
		}
		adr += wt;
	}
...
static PF_Err BlurRedVur(CFsGraph *g, A_long blue, A_u_char *scanline, A_long *tbl)
{
	PF_Err			err = PF_Err_NONE;
	...
	for (A_long x = 0; x < w; x++)
	{
		...
		for (A_long y = 0; y < h; y++) {
			...
			if (count > 0) {
				data[adr].red = RoundByteLong(r / count);
			}
			adr += wt;
		}
	}
...
PF_Err BlurRed08(CFsAE *ae, FlareInfo *infoP)
{
	...
	//scanline
	A_u_char* scanline = *(A_u_char**)infoP->bufH;
...
static PF_Err BlurAllHor(CFsGraph *g, A_long blue, PF_Pixel *scanline, A_long *tbl)
{
	PF_Err			err = PF_Err_NONE;
	...
	for (A_long y = 0; y < h; y++)
	{
		...
		for (A_long x = 0; x <w; x++) {
			...
			if (count > 0) {
				data[adr + x].red = RoundByteLong(r / count);
				data[adr + x].green = RoundByteLong(g / count);
				data[adr + x].blue = RoundByteLong(b / count);
				data[adr + x].alpha = RoundByteLong(a / count);
			}
		}
		adr += wt;
	}
...
static PF_Err BlurAllVur(CFsGraph *g, A_long blue, PF_Pixel *scanline, A_long *tbl)
{
	PF_Err			err = PF_Err_NONE;
	...
	for (A_long x = 0; x < w; x++)
	{
		...
		for (A_long y = 0; y < h; y++) {
			...
			if (count > 0) {
				data[adr].red = RoundByteLong(r / count);
				data[adr].green = RoundByteLong(g / count);
				data[adr].blue = RoundByteLong(b / count);
				data[adr].alpha = RoundByteLong(a / count);
			}
			adr += wt;
		}
	}
...
PF_Err BlurAll08(CFsAE *ae, FlareInfo *infoP)
{
	...
	//scanline
	PF_Pixel* scanline = *(PF_Pixel**)infoP->bufH;
