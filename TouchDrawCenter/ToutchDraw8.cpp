...
/*
	アルファーへ書き込み
*/
inline void PsetAlpha(CFsGraph *g, PsetAlphaPrm *prm)
{
	...
	xx2 = prm->x - (PF_FpLong)x1;
	yy2 = prm->y - (PF_FpLong)y1;
	xx1 = 1 - xx2;
	yy1 = 1 - yy2;

	A_long v = (A_long)(PF_MAX_CHAN8 * opacity);
	A_long target = x1 + (y1 * wt);

	//まず、ぴったりの時
	if ( (xx2 == 0)&&(yy2 == 0) ) {
		data[target].alpha = mixCh8(data[target].alpha,v);
		data[target].red = data[target].alpha;
	}else{
		//まず左上
		if ( ((x1>=0)&&(y1>=0)) == TRUE){
			data[target].alpha = mixCh8(data[target].alpha,(A_long)(v * xx1 * yy1));
			data[target].red = data[target].alpha;
		}
		//右上
		target++;
		if ( ((x2<w)&&(y1>=0)) == TRUE){
			data[target].alpha = mixCh8(data[target].alpha,(A_long)(v * xx2 *yy1));
			data[target].red = data[target].alpha;
		}
		//右下
		target+= wt;
		if ( ((x2<w)&&(y2<h)) == TRUE){
			data[target].alpha = mixCh8(data[target].alpha,(A_long)(v * xx2 * yy2));
			data[target].red = data[target].alpha;
		}
		//左下
		target--;
		if ( ((x1>=0)&&(y2<h)) == TRUE){
			data[target].alpha = mixCh8(data[target].alpha,(A_long)(v * xx1 * yy2));
			data[target].red = data[target].alpha;
		}
	}
...
//-------------------------------------------------------
static void drawTouchSub8(CFsGraph *g, TouchDrawCenterInfo *p)
{
