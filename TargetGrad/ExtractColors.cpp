...
static PF_Err SearchExtractArea8(NF_AE* ae, ParamInfo* info)
{
    A_long width = ae->outputInfo.width;
    A_long height = ae->outputInfo.height;
    A_long widthTrue = ae->outputInfo.widthTrue;
    PF_Pixel8* data = (PF_Pixel8*)ae->output->data;

    for (A_long y = 0; y < height; y++) {
        PF_Pixel8* data2 = data + y * widthTrue;
...
static PF_Err SearchExtractArea16(NF_AE* ae, ParamInfo* info)
{
    A_long width = ae->outputInfo.width;
    A_long height = ae->outputInfo.height;
    A_long widthTrue = ae->outputInfo.widthTrue;
    PF_Pixel16* data = (PF_Pixel16*)ae->output->data;

    for (A_long y = 0; y < height; y++) {
        PF_Pixel16* data2 = data + y * widthTrue;
        for (A_long x = 0; x < width; x++) {
            if (data2[x].alpha > 0) {
                if (x < info->area.left) info->area.left = x;
                if (x > info->area.right) info->area.right = x;
...
static PF_Err SearchExtractArea32(NF_AE* ae, ParamInfo* info)
{
    A_long width = ae->outputInfo.width;
    A_long height = ae->outputInfo.height;
    A_long widthTrue = ae->outputInfo.widthTrue;
    PF_PixelFloat* data = (PF_PixelFloat*)ae->output->data;
    PF_PixelFloat* data2;
    for (A_long y = 0; y < height; y++) {
        data2 = data + y * widthTrue;
        for (A_long x = 0; x < width; x++) {
            if (data2[x].alpha > 0) {
                if (x < info->area.left) info->area.left = x;
                if (x > info->area.right) info->area.right = x;
...
static PF_Err ExtractColorL(NF_AE* ae, ParamInfo* info)
{
	...
	info->area.left = ae->outputInfo.width+8;
	info->area.top = ae->outputInfo.height+8;
	info->area.right = 0-8;
    ...
    case PF_PixelFormat_ARGB32:
        if (info->targetColorMode == 1) {
            ERR(ae->suitesP->Iterate8Suite2()->iterate(in_data, 0, num_rows, input, NULL, &ci, ExtractColor8, output));
        }else {
            ERR(ae->suitesP->Iterate8Suite2()->iterate(in_data, 0, num_rows, input, NULL, &ci, ExtractAlpha8, output));
        }
        ERR(SearchExtractArea8(ae, info));
        break;
    case PF_PixelFormat_ARGB64:
        if (info->targetColorMode == 1) {
            ERR(ae->suitesP->Iterate16Suite2()->iterate(in_data, 0, num_rows, input, NULL, &ci, ExtractColor16, output));
        }else {
            ERR(ae->suitesP->Iterate16Suite2()->iterate(in_data, 0, num_rows, input, NULL, &ci, ExtractAlpha16, output));
        }
        ERR(SearchExtractArea16(ae, info));
        break;
    case PF_PixelFormat_ARGB128:
        if (info->targetColorMode == 1) {
            ERR(ae->suitesP->IterateFloatSuite2()->iterate(in_data, 0, num_rows, input, NULL, &ci, ExtractColorFloat, output));
        }
        else {
            ERR(ae->suitesP->IterateFloatSuite2()->iterate(in_data, 0, num_rows, input, NULL, &ci, ExtractAlphaFloat, output));
        }
