...
//*****************************************************************************
inline PF_Boolean CompPx(thinInfo8 *ti, PF_Pixel p)
{
	PF_Boolean ret = FALSE;
	for (A_long i=0; i<ti->colorMax; i++){
		if (  ( F_ABS((A_long)ti->color[i].red   - (A_long)p.red  ) <= ti->level)
			&&( F_ABS((A_long)ti->color[i].green - (A_long)p.green) <= ti->level)
			&&( F_ABS((A_long)ti->color[i].blue  - (A_long)p.blue ) <= ti->level)){
			ret = TRUE;
			break;
		}
	}
...
//*****************************************************************************
inline PF_Boolean CompPxs(thinInfo8 *ti, PF_Pixel p1,PF_Pixel p2,PF_Pixel p3)
{
	PF_Boolean ret = FALSE;
	if (  ( F_ABS((A_long)p1.red   - (A_long)p2.red  ) <= ti->level)
		&&( F_ABS((A_long)p1.green - (A_long)p2.green) <= ti->level)
		&&( F_ABS((A_long)p1.blue  - (A_long)p2.blue ) <= ti->level)){
		if (  ( F_ABS((A_long)p1.red   - (A_long)p3.red  ) <= ti->level)
			&&( F_ABS((A_long)p1.green - (A_long)p3.green) <= ti->level)
			&&( F_ABS((A_long)p1.blue  - (A_long)p3.blue ) <= ti->level)){
				ret = TRUE;
		}
	}
	return ret;
...
//*****************************************************************************
inline PF_Boolean CompPxs2(thinInfo8 *ti, PF_Pixel p1,PF_Pixel p2)
{
	PF_Boolean ret = FALSE;
	if (  ( F_ABS((A_long)p1.red   - (A_long)p2.red  ) <= ti->level)
		&&( F_ABS((A_long)p1.green - (A_long)p2.green) <= ti->level)
...
//*****************************************************************************
inline PF_Pixel getScanLine(thinInfo8 *ti, A_long offsetX, A_long offsetY)
{
...
//*****************************************************************************
inline PF_Boolean getData(thinInfo8 *ti, A_long offsetX, A_long offsetY)
{
	A_long xx = ti->nowX + offsetX;
	...
	PF_Pixel p = ti->data[ xx + yy*ti->wt]; 
	 
	return CompPx(ti,p);
}
//*****************************************************************************
inline PF_Pixel getPx(thinInfo8 *ti, A_long offsetX, A_long offsetY)
{
	A_long xx = ti->nowX + offsetX;
	A_long yy = ti->nowY + offsetY;
	if (xx<0) {
...
//*****************************************************************************
inline PF_Pixel minColor(PF_Pixel s,PF_Pixel d)
{
...
//*****************************************************************************
inline thinBak8 getPatEdge(thinInfo8 *ti)
{
	thinBak8 ret;
	ret.drawFlag = FALSE;
...
//*****************************************************************************
//*****************************************************************************
inline thinBak8 getPatDot(thinInfo8 *ti)
{
	thinBak8 ret;
	ret.drawFlag = FALSE;
...
//*****************************************************************************
//*****************************************************************************
inline thinBak8 getPatDot2nd(thinInfo8 *ti)
{
	thinBak8 ret;
	ret.drawFlag = FALSE;
	ret.dir = -1;
...
//*****************************************************************************
//*****************************************************************************
inline thinBak8 getPat(thinInfo8 *ti)
{
	thinBak8 ret;
	ret.drawFlag = FALSE;
	...
	*/
	if (patCount>7) {
		return ret;
			...
			ret.drawFlag = TRUE;
			PF_Pixel c = minColor(table[1],table[3]);
...
PF_Err thinExec8(CFsAE *ae , ParamInfo *infoP)
{
	...
	//メモリの確保
	ti.scanlineH = ae->NewHandle(ti.wt * sizeof(PF_Pixel) * 4);// 3Line分　と余裕
	if ( !ti.scanlineH ) return PF_Err_INTERNAL_STRUCT_DAMAGED;

	ti.scanline = *(PF_Pixel8**)ti.scanlineH; ;
	ti.data = (PF_Pixel *)ae->out->data();
	ti.white = infoP->white;
	ti.alphaZero = infoP->alphaZero;
	ti.edge = infoP->edge;

	PF_Pixel kk = {0,0,0,0};
	PF_Pixel k = {128,128,128,128};
	PF_Pixel rd = {255,255,0,0};
	A_long now = 0;
	//exec
		...
		if (ti.edge==TRUE){
			for (A_long j = 0; j<ti.h; j++)
			{
				...
				for ( A_long i = 0; i<ti.w; i++) {
					ti.nowX = i; 
					PF_Pixel p = getScanLine(&ti,0,0);
					if ( CompPx(&ti,p) ==TRUE ) {
						thinBak8 tb = getPatEdge(&ti);
						if (tb.drawFlag ==TRUE) {
							if (ti.white == TRUE){
								if ((tb.color.blue == PF_MAX_CHAN8)
									&&(tb.color.green == PF_MAX_CHAN8)
									&&(tb.color.red == PF_MAX_CHAN8)
									&&(tb.color.alpha == PF_MAX_CHAN8))
								{
									tb.drawFlag = FALSE;
								}
							}
		...
		for (A_long j = 0; j<ti.h; j++)
		{
			...
			for ( A_long i = 0; i<ti.w; i++) {
				...
				if ( CompPx(&ti,p) ==TRUE ) {
					...
					if (tb.drawFlag ==TRUE) {
						PF_Pixel p = ti.data[now2];
						if (ti.white == TRUE) {
							if ((p.blue == PF_MAX_CHAN8)
								&&(p.green == PF_MAX_CHAN8)
								&&(p.red == PF_MAX_CHAN8)
