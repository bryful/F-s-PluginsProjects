...
struct DirectXGPUData
{
	...
	ShaderObjectPtr mInvertShader;
	ShaderObjectPtr mProcAmpShader;
};
#endif
	...
	PF_GPUDeviceSetupExtra *extraP)
{
	...
	else if (extraP->input->what_gpu == PF_GPU_Framework_DIRECTX)
	{
		...
		DX_ERR(GetShaderPath(L"InvertColorKernel", csoPath, sigPath));

		// Load the shaders
		DX_ERR(dx_gpu_data->mContext->LoadShader(
			csoPath.c_str(),
			sigPath.c_str(),
			dx_gpu_data->mInvertShader));

		DX_ERR(GetShaderPath(L"ProcAmp2Kernel", csoPath, sigPath));

		DX_ERR(dx_gpu_data->mContext->LoadShader(
			csoPath.c_str(),
			sigPath.c_str(),
			dx_gpu_data->mProcAmpShader));
	...
	if (!err && extraP->input->what_gpu == PF_GPU_Framework_OPENCL)
	{
		...
		else if (!err && extraP->input->what_gpu == PF_GPU_Framework_DIRECTX)
		{
...

			{
				...
				DX_ERR(shaderExecution.SetParamBuffer(&invert_params, sizeof(InvertColorParams)));
				DX_ERR(shaderExecution.SetUnorderedAccessView(
					(ID3D12Resource*)im_mem,
					invert_params.mHeight * tmp_row_bytes));
				DX_ERR(shaderExecution.SetShaderResourceView(
					(ID3D12Resource*)src_mem,
					invert_params.mHeight* src_row_bytes));
				DX_ERR(shaderExecution.Execute((UINT)DivideRoundUp(invert_params.mWidth, 16), (UINT)DivideRoundUp(invert_params.mHeight, 16)));
			}
			if (!err)
			{
				// Execute ProcAmp
				...
				// Note: The order of elements in the param structure should be identical to the order expected by the shader
				DX_ERR(shaderExecution.SetParamBuffer(&procamp_params, sizeof(ProcAmp2Params)));
				DX_ERR(shaderExecution.SetUnorderedAccessView(
					(ID3D12Resource*)dst_mem,
					procamp_params.mHeight * dst_row_bytes));
				DX_ERR(shaderExecution.SetShaderResourceView(
					(ID3D12Resource*)im_mem,
					procamp_params.mHeight* tmp_row_bytes));
				DX_ERR(shaderExecution.Execute((UINT)DivideRoundUp(procamp_params.mWidth, 16), (UINT)DivideRoundUp(procamp_params.mHeight, 16)));
			}
		}
	#endif
	#if HAS_METAL
