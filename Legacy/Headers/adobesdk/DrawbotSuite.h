...
#define DRAWBOT_DrawbotSuiteCurrent DRAWBOT_DrawbotSuite1
    ...
    typedef struct DRAWBOT_DrawbotSuite1
    {
        ...
        SPAPI SPErr (*GetSupplier)(DRAWBOT_DrawRef in_drawbot_ref, DRAWBOT_SupplierRef* out_supplierP);

        //Get the surface from the drawbot_ref
        SPAPI SPErr (*GetSurface)(DRAWBOT_DrawRef in_drawbot_ref, DRAWBOT_SurfaceRef* out_surfaceP);
...
#define DRAWBOT_SupplierSuiteCurrent DRAWBOT_SupplierSuite1
    ...
    typedef struct DRAWBOT_SupplierSuite1
    {
        ...
        //It should be released with ReleaseObject api.
        SPErr (*NewBrush)(
            DRAWBOT_SupplierRef in_supplier_ref, const DRAWBOT_ColorRGBA* in_colorP, DRAWBOT_BrushRef* out_brushP);

        //Check if current supplier supports text.
        SPErr (*SupportsText)(DRAWBOT_SupplierRef in_supplier_ref, DRAWBOT_Boolean* out_supports_textPB);

        //Get default font size.
        SPErr (*GetDefaultFontSize)(DRAWBOT_SupplierRef in_supplier_ref, float* out_font_sizeF);

        //Create a new font with default settings.
        //You can pass default font size from GetDefaultFontSize.
        //It should be released with ReleaseObject api.
        SPErr (*NewDefaultFont)(DRAWBOT_SupplierRef in_supplier_ref, float in_font_sizeF, DRAWBOT_FontRef* out_fontP);

        //Create a new image from buffer.
        //It should be released with ReleaseObject api.
        SPErr (*NewImageFromBuffer)(
            DRAWBOT_SupplierRef in_supplier_ref,
            int in_width,
            int in_height,
            int in_row_bytes,
        ...
        //Create a new path.
        //It should be released with ReleaseObject api.
        SPErr (*NewPath)(DRAWBOT_SupplierRef in_supplier_ref, DRAWBOT_PathRef* out_pathP);
        ...
        SPErr (*SupportsPixelLayoutBGRA)(DRAWBOT_SupplierRef in_supplier_ref, DRAWBOT_Boolean* out_supports_bgraPB);

        SPErr (*PrefersPixelLayoutBGRA)(DRAWBOT_SupplierRef in_supplier_ref, DRAWBOT_Boolean* out_prefers_bgraPB);

        SPErr (*SupportsPixelLayoutARGB)(DRAWBOT_SupplierRef in_supplier_ref, DRAWBOT_Boolean* out_supports_argbPB);

        SPErr (*PrefersPixelLayoutARGB)(DRAWBOT_SupplierRef in_supplier_ref, DRAWBOT_Boolean* out_prefers_argbPB);
...
#define DRAWBOT_SurfaceSuiteCurrent DRAWBOT_SurfaceSuite2
    ...
    typedef struct DRAWBOT_SurfaceSuite1
    {
        ...
        SPErr (*PaintRect)(
            DRAWBOT_SurfaceRef in_surface_ref, const DRAWBOT_ColorRGBA* in_colorP, const DRAWBOT_RectF32* in_rectPR);
        ...
        SPErr (*StrokePath)(DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_PenRef in_pen_ref, DRAWBOT_PathRef in_path_ref);

        //Clip the surface
        SPErr (*Clip)(
            DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_SupplierRef in_supplier_ref, const DRAWBOT_Rect32* in_rectPR);

        //Get clip bounds
        SPErr (*GetClipBounds)(DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_Rect32* out_rectPR);

        //Checks whether a rect is within the clip bounds.
        SPErr (*IsWithinClipBounds)(
            DRAWBOT_SurfaceRef in_surface_ref, const DRAWBOT_Rect32* in_rectPR, DRAWBOT_Boolean* out_withinPB);

        //Transform the last surface state.
        SPErr (*Transform)(DRAWBOT_SurfaceRef in_surface_ref, const DRAWBOT_MatrixF32* in_matrixP);
        ...
        SPErr (*SetInterpolationPolicy)(DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_InterpolationPolicy in_interp);

        SPErr (*GetInterpolationPolicy)(DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_InterpolationPolicy* out_interpP);

        SPErr (*SetAntiAliasPolicy)(DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_AntiAliasPolicy in_policy);

        SPErr (*GetAntiAliasPolicy)(DRAWBOT_SurfaceRef in_surface_ref, DRAWBOT_AntiAliasPolicy* out_policyP);

        //Flush drawing
        SPErr (*Flush)(DRAWBOT_SurfaceRef in_surface_ref);

        //Get the transform of the surface state.
        SPErr (*GetTransformToScreenScale)(DRAWBOT_SurfaceRef in_surface_ref, float* out_scale);
...
#define DRAWBOT_PathSuiteCurrent DRAWBOT_PathSuite1
    ...
    typedef struct DRAWBOT_PathSuite1
    {
        ...
        //Move to a point
        SPErr (*MoveTo)(DRAWBOT_PathRef in_path_ref, float in_x, float in_y);

        //Add a line to the path
        SPErr (*LineTo)(DRAWBOT_PathRef in_path_ref, float in_x, float in_y);
        ...
        SPErr (*AddRect)(DRAWBOT_PathRef in_path_ref, const DRAWBOT_RectF32* in_rectPR);
...
#define DRAWBOT_PenSuiteCurrent DRAWBOT_PenSuite1
    ...
    typedef struct DRAWBOT_PenSuite1
    {
        ...
        SPErr (*SetDashPattern)(DRAWBOT_PenRef in_pen_ref, const float* in_dashesP, int in_pattern_size);
...
#define DRAWBOT_ImageSuiteCurrent DRAWBOT_ImageSuite1
    ...
    typedef struct DRAWBOT_ImageSuite1
    {
        ...
        //Make the line dashed
        SPErr (*SetScaleFactor)(DRAWBOT_ImageRef in_image_ref, float in_scale_factor);
...
public:
    ...
    #define DRAWBOT_ErrorToException(EXPR)                                                                             \
        {                                                                                                              \
            SPErr _err = (EXPR);                                                                                       \
            if (_err)                                                                                                  \
                throw DRAWBOT_Exception(_err);                                                                         \
        }
...
public:
    DRAWBOT_PathP(DRAWBOT_SupplierSuiteCurrent* suiteP, const DRAWBOT_SupplierRef supplier_ref)
        : _inherited(suiteP)
    {
        New(suiteP, supplier_ref);
    }

    void New(DRAWBOT_SupplierSuiteCurrent* suiteP, const DRAWBOT_SupplierRef supplier_ref)
    {
        DRAWBOT_PathRef path_ref;
...
public:
    DRAWBOT_BrushP(
        DRAWBOT_SupplierSuiteCurrent* suiteP, const DRAWBOT_SupplierRef supplier_ref, const DRAWBOT_ColorRGBA* colorP)
        : _inherited(suiteP)
    {
...
public:
    DRAWBOT_FontP(DRAWBOT_SupplierSuiteCurrent* suiteP, const DRAWBOT_SupplierRef supplier_ref, float in_font_size)
        : _inherited(suiteP)
    {
        New(suiteP, supplier_ref, in_font_size);
    }

    void New(DRAWBOT_SupplierSuiteCurrent* suiteP, const DRAWBOT_SupplierRef supplier_ref, float in_font_size)
    {
...
{
public:
    DRAWBOT_SaveAndRestoreStateStack(DRAWBOT_SurfaceSuiteCurrent* suiteP, const DRAWBOT_SurfaceRef surface_ref)
