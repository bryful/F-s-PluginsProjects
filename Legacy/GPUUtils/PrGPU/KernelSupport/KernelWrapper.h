...
// clang-format off
#define GF_SIGNATURE_HLSL(name, positions)          \
	KERNEL_DELEGATE_NAME(name)(						\
		SEQ_PAIRS_ENUM(positions))
// clang-format on

// clang-format off
#define GF_UNMARSHAL_USING_GLOBALS(name, buffers, values, positions, shares)	\
	KERNEL_WRAPPER_CAT(name,__HLSL_ENTRYPOINT_FILE_);							\
	GF_DEVICE_FUNCTION void GF_SIGNATURE_HLSL(name, positions);				    \
	SEQ_PAIRS(buffers,__HLSL_REGISTER;)											\
	KERNEL_DECLARE_SHARES(shares)											    \
	cbuffer cb__HLSL_REGISTER {													\
		SEQ_PAIRS(values, ;)													\
	};																			\
...
// clang-format on
    ...
    #define GF_UNMARSHAL GF_UNMARSHAL_USING_GLOBALS
#else
...
#elif GF_DEVICE_TARGET_HLSL
    #define KERNEL_XY IDUnsigned(dispatchThreadId)
    #define THREAD_ID IDUnsigned(groupThreadId)
    #define BLOCK_ID IDUnsigned(groupID)
    #define BLOCK_SIZE make_uint2(__HLSL_BLOCK_SIZE_X, __HLSL_BLOCK_SIZE_Y)
GF_DEVICE_FUNCTION uint2 IDUnsigned(uint3 inXYZ)
{
    return make_uint2(inXYZ.x, inXYZ.y);
}
#else
...
{
    return make_uint2(
        GF_FASTMULTIPLY(blockIdx.x, blockDim.x) + threadIdx.x, GF_FASTMULTIPLY(blockIdx.y, blockDim.y) + threadIdx.y);
}
