...
inline PF_PixelFloat PxPer(PF_PixelFloat p, PF_FpLong per)
{
	...
	r.alpha = RoundFpShortDouble((PF_FpLong)p.alpha * per);
	return r;
}
//-------------------------------------------------------------------------------------------------
//小数点対応のスクリーン合成
	...
	PF_PixelFloat	*outP)
{
	...
	PF_FpLong a = RoundFpShort2(inP->alpha);

	outP->red = scr(outP->red, (PF_FpShort)((PF_FpLong)inP->red*a));
	outP->green = scr(outP->green, (PF_FpShort)((PF_FpLong)inP->green*a));
	outP->blue = scr(outP->blue, (PF_FpShort)((PF_FpLong)inP->blue*a));
	...
	PF_PixelFloat	*outP)
{
	...
	outP->alpha = scr(RoundFpShort2(outP->alpha), RoundFpShort2(inP->alpha));

	PF_FpLong a = RoundFpShort2(inP->alpha);

	outP->red = RoundFpShortDouble(outP->red + (inP->red*a));
	outP->green = RoundFpShortDouble(outP->green + (inP->green*a));
	outP->blue = RoundFpShortDouble(outP->blue + (inP->blue*a));
	...
	PF_PixelFloat	*outP)
{
	...
	PF_FpLong ar = 1 - RoundFpShort2(outP->alpha);
	PF_FpLong a = RoundFpShort2(inP->alpha);
	outP->alpha = scr(outP->alpha, inP->alpha);
	...
	outP->red = RoundFpShortDouble((PF_FpLong)outP->red + r );
	outP->green = RoundFpShortDouble((PF_FpLong)outP->green + g);
	outP->blue = RoundFpShortDouble((PF_FpLong)outP->blue + b);
	...
	PF_PixelFloat	*outP)
{
	...
	c.red = RoundFpShortDouble(inP->red * a);
	c.green = RoundFpShortDouble(inP->green * a);
	c.blue = RoundFpShortDouble(inP->blue * a);
...
//-------------------------------------------------------------------------------------------------
static PF_Err
CopyAtoTemp(ParamInfo32 *infoP)
{
	PF_Err			err = PF_Err_NONE;

	PF_PixelFloat * dataO = (PF_PixelFloat *)infoP->gout.data;
	PF_PixelFloat * dataT = (PF_PixelFloat *)infoP->gtmp.data;
	...
	for (A_long y = 0; y < infoP->gout.height; y++)
	{
		...
		{
			PF_FpShort a = dataO[x + horO].alpha;
			dataT[x + horT].alpha = 1;
			dataT[x + horT].red = a;
...
TempRev(ParamInfo32 *infoP)
{
	...
	for (A_long y = 0; y < gi.height; y++)
	{
		...
		{
			A_long pos = x + hor;
			data[pos].red = 1 - data[pos].red;
			data[pos].green = 1 - data[pos].green;
		}
		hor += gi.widthTrue;
	}
...
TempSub(ParamInfo32 *infoP)
{
	...
	for (A_long y = 0; y < gi.height; y++)
	{
		for (A_long x = 0; x < gi.width; x++)
		{
			pos = x + hor;
			data[pos].green = 1 - data[pos].green;
			data[pos].red = RoundFpShortDouble(data[pos].blue - data[pos].green);
		}
		hor += gi.widthTrue;
	}
	...
	for (A_long y = 0; y < gi.height; y++)
	{
		for (A_long x = 0; x < gi.width; x++) 
		{
			...
			if (org < mx1) {
				org += mx1/2;
				if (org > mx1) org = mx1;
				data[pos].green = RoundFpShortDouble(org);
			}
...
//-------------------------------------------------------------------------------------------------
static void
MakePixelTable(PF_PixelFloat *tbl, A_long len, PF_PixelFloat col, PF_FpLong per, PF_FpLong br)
{
	...
	for (A_long i = 0; i < len2; i++)
	{
		...
		tbl[i].red = RoundFpShortDouble(tbl[i].red + (r * br * (len2 - i) / len2));
		tbl[i].green = RoundFpShortDouble(tbl[i].green + (g * br * (len2 - i) / len2));
		tbl[i].blue = RoundFpShortDouble(tbl[i].blue + (b * br * (len2 - i) / len2));

	}

}
//-------------------------------------------------------------------------------------------------
static PF_Err StarDrawColor(ParamInfo32 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	PF_PixelFloat *dataT = (PF_PixelFloat *)gt.data;
	...
	for (A_long y = 0; y < go.height; y++)
	{
		...
		{
			lv = RoundFpShort2( dataT[posT].red);
			posT++;
			if (lv <= 0) continue;
			...
			{
				A_long vl = (A_long)(pi->ver.len * lv+0.5);
				if (vl > 0)
				{
			...
			{
				A_long hl = (A_long)(pi->hor.len * lv+0.5);
				if (hl > 0)
				{
			...
			{
				A_long dl = (A_long)(pi->dia.len * lv +0.5);
				if (dl > 0)
				{
...
//-------------------------------------------------------------------------------------------------
static PF_Err CenterDraw(ParamInfo32 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	PF_PixelFloat *dataO = (PF_PixelFloat *)go.data;
	PF_PixelFloat *dataT = (PF_PixelFloat *)gt.data;
	...
	r = (r + (1 - r)*pi->info.brigthness);
	g = (g + (1 - g)*pi->info.brigthness);
	b = (b + (1 - b)*pi->info.brigthness);
	for (A_long y = 0; y < go.height; y++)
	{
		for (A_long x = 0; x < go.width; x++)
		{
			PF_FpLong v = (PF_FpLong)dataT[posT].green;
			dataO[posO].red = RoundFpShortDouble(dataO[posO].red + r * v);
			dataO[posO].green = RoundFpShortDouble(dataO[posO].green + g * v);
			dataO[posO].blue = RoundFpShortDouble(dataO[posO].blue + b * v);
			dataO[posO].alpha = RoundFpShortDouble( dataO[posO].alpha + dataT[posT].green);
...
//-------------------------------------------------------------------------------------------------
static PF_Err blur(ParamInfo32 *pi)
{
	PF_Err			err = PF_Err_NONE;

	GInfo go = pi->gout;

	PF_PixelFloat *data = (PF_PixelFloat *)go.data;
...
//-------------------------------------------------------------------------------------------------
static PF_Err blurTempRed(ParamInfo32 *pi)
{
#define DEC1 16
#define DEC2 32
	PF_Err			err = PF_Err_NONE;
	...
	PF_PixelFloat *data = (PF_PixelFloat *)gt.data;
	...
	for (A_long y = 0; y < gt.height; y++)
	{
		for (A_long x = 0; x < gt.width; x++)
		{
			...
			data[pos].red = RoundFpShortDouble(r);
			pos++;
...
//-------------------------------------------------------------------------------------------------
static PF_Err ScreenTemp(ParamInfo32 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	PF_PixelFloat *dataI = (PF_PixelFloat *)gi.data;
	PF_PixelFloat *dataT = (PF_PixelFloat *)gt.data;
...
//-------------------------------------------------------------------------------------------------
static PF_Err StarDrawScreen(ParamInfo32 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	for (A_long y = 0; y < go.height; y++)
	{
		for (A_long x = 0; x < go.width; x++)
		{
			PF_PixelFloat c = dataT[posT];
			lv = c.alpha;
			lvb = (PF_FpLong)c.alpha;
			posT++;
			if (lv <= 0) continue;
...
//-------------------------------------------------------------------------------------------------
static void
MakeRainbowTable(PF_PixelFloat *tbl, A_long len,  PF_FpLong ro, PF_FpLong rd, PF_FpLong per, PF_FpLong br)
{
...
//-------------------------------------------------------------------------------------------------
static PF_Err StarDrawRainbow(ParamInfo32 *pi)
{
	PF_Err			err = PF_Err_NONE;
...
//-------------------------------------------------------------------------------------------------
static PF_Err ToParam(CFsAE *ae, ParamInfo *infoP, ParamInfo32 *pi)
{
	PF_Err	err = PF_Err_NONE;
	...
	pi->color = CONV8TO32(infoP->color);
...
PF_Err StarExec32(CFsAE *ae, ParamInfo *infoP)
{
	...
	ToParam(ae, infoP, &pi);
	pi.bufH = ae->NewHandle( (pi.hor.len + pi.ver.len + pi.dia.len)* sizeof(PF_PixelFloat)+ 1024);
	if (pi.bufH == NULL) {
		err = PF_Err_OUT_OF_MEMORY;
		return err;
	}
	pi.horTable = *(PF_PixelFloat**)(pi.bufH);
	pi.verTable = (pi.horTable + pi.hor.len);
	pi.diaTable = (pi.verTable + pi.ver.len);
	//ターゲットエリア
	ERR(ae->iterate32((refconType)&pi, TargetPixel));
	...
	if (pi.info.target_maskDraw==TRUE) {
		...
		{
		case 4:
			ERR(ae->iterate32((refconType)&pi, BlendNormal));
			break;
		case 3:
			ERR(ae->iterate32((refconType)&pi, BlendAdd));
			break;
		case 2:
			ERR(ae->iterate32((refconType)&pi, BlendScreen));
			break;
		case 1:
		default:
			break;
