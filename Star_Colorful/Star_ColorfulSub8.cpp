...
{
	PF_Pixel r;
	r.red = RoundByteFpLong( (PF_FpLong)p.red * per);
	r.green = RoundByteFpLong((PF_FpLong)p.green * per);
	r.blue = RoundByteFpLong((PF_FpLong)p.blue * per);
	r.alpha = RoundByteFpLong((PF_FpLong)p.alpha * per);
	return r;
}
//-------------------------------------------------------------------------------------------------
//小数点対応のスクリーン合成
	...
	PF_Pixel8	*outP)
{
	...
	outP->red = scr(outP->red, (A_u_char)((PF_FpLong)inP->red*a));
	outP->green = scr(outP->green, (A_u_char)((PF_FpLong)inP->green*a));
	outP->blue = scr(outP->blue, (A_u_char)((PF_FpLong)inP->blue*a));
	...
	PF_Pixel8	*outP)
{
	...
	outP->red = RoundByteLong((A_long)outP->red + (A_long)((PF_FpLong)inP->red*a));
	outP->green = RoundByteLong((A_long)outP->green + (A_long)((PF_FpLong)inP->green*a));
	outP->blue = RoundByteLong((A_long)outP->blue + (A_long)((PF_FpLong)inP->blue*a));
	...
	PF_Pixel8	*outP)
{
	...
	PF_FpLong ar = 1 - (PF_FpLong)outP->alpha/ PF_MAX_CHAN8;
	outP->alpha = scr(outP->alpha, inP->alpha);
	...
	outP->red = RoundByteFpLong((PF_FpLong)outP->red + r );
	outP->green = RoundByteFpLong((PF_FpLong)outP->green + g);
	outP->blue = RoundByteFpLong((PF_FpLong)outP->blue + b);
	...
	PF_Pixel8	*outP)
{
	...
	PF_FpShort  r, g, b, a;
	PF_FpLong v = 0;
	a = (PF_FpShort)inP->alpha / PF_MAX_CHAN8;
	r = (PF_FpShort)inP->red * a / PF_MAX_CHAN8;
	g = (PF_FpShort)inP->green * a / PF_MAX_CHAN8;
	b = (PF_FpShort)inP->blue * a / PF_MAX_CHAN8;
	...
	c.alpha = RoundByteLong((A_long)(a*PF_MAX_CHAN8 + 0.5));

	c.red = RoundByteLong((A_long)((double)inP->red * a+0.5));
	c.green = RoundByteLong((A_long)((double)inP->green * a + 0.5));
	c.blue = RoundByteLong((A_long)((double)inP->blue * a + 0.5));
...
//-------------------------------------------------------------------------------------------------
static PF_Err
CopyAtoTemp(ParamInfo8 *infoP)
{
	PF_Err			err = PF_Err_NONE;

	PF_Pixel * dataO = (PF_Pixel *)infoP->gout.data;
	PF_Pixel * dataT = (PF_Pixel *)infoP->gtmp.data;
	...
	for (A_long y = 0; y < infoP->gout.height; y++)
	{
		...
		{
			A_u_char a = dataO[x + horO].alpha;
			dataT[x + horT].alpha = PF_MAX_CHAN8;
			dataT[x + horT].red = a;
...
TempRev(ParamInfo8 *infoP)
{
	...
	for (A_long y = 0; y < gi.height; y++)
	{
		...
		{
			A_long pos = x + hor;
			data[pos].red = PF_MAX_CHAN8 - data[pos].red;
			data[pos].green = PF_MAX_CHAN8 - data[pos].green;
		}
		hor += gi.widthTrue;
	}
...
TempSub(ParamInfo8 *infoP)
{
	...
	for (A_long y = 0; y < gi.height; y++)
	{
		for (A_long x = 0; x < gi.width; x++)
		{
			pos = x + hor;
			data[pos].green = PF_MAX_CHAN8 - data[pos].green;
			data[pos].red = RoundByteLong(data[pos].blue - data[pos].green);
			//data[pos].green = data[pos].red;
		}
		hor += gi.widthTrue;
	}
	...
	for (A_long y = 0; y < gi.height; y++)
	{
		for (A_long x = 0; x < gi.width; x++) 
		{
			...
			if (org < mx1) {
				org += mx1/2;
				if (org > mx1) org = mx1;
				data[pos].green = RoundByteLong(org);
			}
...
MakePixelTable(PF_Pixel *tbl, A_long len, PF_Pixel col, PF_FpLong per, PF_FpLong br)
{
	...
	{
		A_long r, g, b;
		r = PF_MAX_CHAN8 - tbl[i].red;
		g = PF_MAX_CHAN8 - tbl[i].green;
		b = PF_MAX_CHAN8 - tbl[i].blue;

		tbl[i].red = RoundByteLong(tbl[i].red + (A_long)((PF_FpLong)r * br * (len2 - i) / len2));
		tbl[i].green = RoundByteLong(tbl[i].green + (A_long)((PF_FpLong)g * br * (len2 - i) / len2));
		tbl[i].blue = RoundByteLong(tbl[i].blue + (A_long)((PF_FpLong)b * br * (len2 - i) / len2));

	}

}
//-------------------------------------------------------------------------------------------------
static PF_Err StarDrawColor(ParamInfo8 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	PF_Pixel *dataT = (PF_Pixel *)gt.data;
...
//-------------------------------------------------------------------------------------------------
static PF_Err CenterDraw(ParamInfo8 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	PF_Pixel *dataO = (PF_Pixel *)go.data;
	PF_Pixel *dataT = (PF_Pixel *)gt.data;
	...
	r = (r + (PF_MAX_CHAN8 - r)*pi->info.brigthness);
	g = (g + (PF_MAX_CHAN8 - g)*pi->info.brigthness);
	b = (b + (PF_MAX_CHAN8 - b)*pi->info.brigthness);
	for (A_long y = 0; y < go.height; y++)
	{
		for (A_long x = 0; x < go.width; x++)
		{
			PF_FpLong v = (PF_FpLong)dataT[posT].green / PF_MAX_CHAN8;
			dataO[posO].red = RoundByteFpLong(dataO[posO].red + r * v);
			dataO[posO].green = RoundByteFpLong(dataO[posO].green + g * v);
			dataO[posO].blue = RoundByteFpLong(dataO[posO].blue + b * v);
			dataO[posO].alpha = RoundByteLong( (A_long)dataO[posO].alpha + (A_long)dataT[posT].green);
...
//-------------------------------------------------------------------------------------------------
static PF_Err blur(ParamInfo8 *pi)
{
	PF_Err			err = PF_Err_NONE;

	GInfo go = pi->gout;

	PF_Pixel *data = (PF_Pixel *)go.data;
	...
	for (A_long y = 0; y < go.height; y++)
	{
		for (A_long x = 0; x < go.width; x++)
		{
			...
			c.red = RoundByteFpLong(r);
			c.green = RoundByteFpLong(g);
			c.blue = RoundByteFpLong(b);
			c.alpha = RoundByteFpLong(a);
...
//-------------------------------------------------------------------------------------------------
static PF_Err blurTempRed(ParamInfo8 *pi)
{
#define DEC1 16
#define DEC2 32
	PF_Err			err = PF_Err_NONE;
	...
	PF_Pixel *data = (PF_Pixel *)gt.data;
	...
	for (A_long y = 0; y < gt.height; y++)
	{
		for (A_long x = 0; x < gt.width; x++)
		{
			...
			data[pos].red = RoundByteFpLong(r);
			pos++;
...
static PF_Err ScreenTemp(ParamInfo8 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	PF_Pixel *dataI = (PF_Pixel *)gi.data;
	PF_Pixel *dataT = (PF_Pixel *)gt.data;
...
//-------------------------------------------------------------------------------------------------
static PF_Err StarDrawScreen(ParamInfo8 *pi)
{
	PF_Err			err = PF_Err_NONE;
	...
	for (A_long y = 0; y < go.height; y++)
	{
		for (A_long x = 0; x < go.width; x++)
		{
			PF_Pixel c = dataT[posT];
			lv = c.alpha;
			lvb = (PF_FpLong)c.alpha / PF_MAX_CHAN8;
			posT++;
			if (lv <= 0) continue;
...
//-------------------------------------------------------------------------------------------------
static void
MakeRainbowTable(PF_Pixel *tbl, A_long len,  PF_FpLong ro, PF_FpLong rd, PF_FpLong per, PF_FpLong br)
{
	...
	for (A_long i = 0; i < len; i++)
	{
		...
		hls.L = 0.5;
		hls.A = 1.0;
		PF_Pixel c = CONV32TO8(HLStoRGB(hls));
		tbl[i] = PxPer(c, p);
	}
...
//-------------------------------------------------------------------------------------------------
static PF_Err StarDrawRainbow(ParamInfo8 *pi)
{
	PF_Err			err = PF_Err_NONE;
...
PF_Err StarExec8(CFsAE *ae, ParamInfo *infoP)
{
	...
	pi.bufH = ae->NewHandle( (pi.hor.len + pi.ver.len + pi.dia.len)* sizeof(PF_Pixel)+ 1024);
	if (pi.bufH == NULL) {
	...
	if (pi.info.target_maskDraw==TRUE) {
		...
		{
		case 4:
			ERR(ae->iterate8((refconType)&pi, BlendNormal));
			break;
		case 3:
			ERR(ae->iterate8((refconType)&pi, BlendAdd));
			break;
		case 2:
			ERR(ae->iterate8((refconType)&pi, BlendScreen));
			break;
		case 1:
		default:
			break;
