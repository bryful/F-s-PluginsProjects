...
} LineInfo16, * LineInfo16P, ** LineInfo16H;

//*****************************************************************************
static PF_Err Line16(LineInfo16 *li)
{
...
//*****************************************************************************
static PF_Err drawGraph16(CFsAE* ae, ParamInfo16* infoP)
{
	PF_Err	err = PF_Err_NONE;
	LineInfo16 li;
	li.data = (PF_Pixel16 *)ae->out->data();
	li.width = ae->out->width();
	li.widthTrue = ae->out->widthTrue();
	li.height = ae->out->height();
	li.col = { PF_MAX_CHAN16,PF_MAX_CHAN16, PF_MAX_CHAN16, PF_MAX_CHAN16 };
...
//*****************************************************************************
static PF_Err sampling16(CFsAE* ae, ParamInfo16* infoP)
{
	PF_Err	err = PF_Err_NONE;

	PF_Pixel16* D = (PF_Pixel16*)ae->output->data;
	...
	for (A_long y = 0; y < infoP->SYCount; y++)
	{
		...
		for (A_long x = 0; x < infoP->SXCount; x++)
		{
			double v = (double)D[target].red / PF_MAX_CHAN16;
			target++;
...
//*****************************************************************************
static PF_Err gray16(CFsAE* ae, ParamInfo16* infoP)
{
	PF_Err	err = PF_Err_NONE;

	PF_Pixel16* iD = (PF_Pixel16*)ae->input->data;
	A_long iWt = ae->in->widthTrue();
	A_long iWo = ae->in->offsetWidth();
	A_long iW = ae->in->width();
	A_long iH = ae->in->height();

	PF_Pixel16* oD = (PF_Pixel16*)ae->output->data;
	...
	for (A_long y = 0; y < iH; y++) {
		for (A_long x = 0; x < iW; x++) {
			...
			if (iD[iTarget].alpha < PF_MAX_CHAN16) {
				a *= (double)iD[iTarget].alpha / PF_MAX_CHAN16;
			}
			A_long aa = (A_long)(a + 0.5);
			if (aa > PF_MAX_CHAN16) aa = PF_MAX_CHAN16;
			oD[oTarget].alpha = PF_MAX_CHAN16;
			oD[oTarget].red = oD[oTarget].green = oD[oTarget].blue = (A_u_short)aa;
			iTarget++;
			oTarget++;
		}
		iTarget += iWo;
...
PF_Err exec16(CFsAE* ae, ParamInfo16* infoP)
{
	...
	if ((infoP->mode == MODE_ALL) || (infoP->mode == MODE_WAVE)) {
		ERR(gray16(ae, infoP));
		ERR(sampling16(ae, infoP));
	}
	ERR(ae->out->clear());
	ERR(drawGraph16(ae, infoP));
