...
typedef struct edgeBlurPrmSub32
{
	A_long		blur;
	A_long		offset;
	A_long		tbl[451];
...
//----------------------------------------------------------------------------
PF_Boolean makeTbl(edgeBlurPrm *prm, edgeBlurPrmSub32 *sub)
{
	PF_InData *in_data;
	in_data = prm->in_data;
...
//----------------------------------------------------------------------------
void horMin(edgeBlurPrmSub32 *sub)
{
	A_long hor =0;
	A_long i;
	A_long x=0, y=0;
...
//----------------------------------------------------------------------------
void verMin(edgeBlurPrmSub32 *sub)
{
	A_long hor =0;
	A_long i;
	A_long x=0, y=0;
...
void horBlur(edgeBlurPrmSub32 *sub)
{
	A_long hor =0;
	A_long ir,il,i;
	A_long x=0, y=0;
	...
	for ( y=0; y<sub->height; y++)
	{
		...
		for ( x=0; x<sub->width; x++){
			...
			if (count>0){
				sub->data[hor + x].alpha = RoundFpShortDouble(sa/count);
			}
		}
	}
...
//----------------------------------------------------------------------------
void verBlur(edgeBlurPrmSub32 *sub)
{
	A_long hor =0;
	A_long ir,il,i;
	A_long x=0, y=0;
	...
	for ( x=0; x<sub->width; x++)
	{
		...
		for ( y=0; y<sub->height; y++){
			...
			if (count>0){
				sub->data[hor].alpha = RoundFpShortDouble(sa/count);
			}
			hor += sub->widthTrue;
		}
	}
...
PF_Err EdgeBlurSub32(edgeBlurPrm *prm)
{
	...
	edgeBlurPrmSub32 sub;
	sub.data = (PF_PixelFloat *)prm->data;
	sub.scanline =*(PF_FpShort **)prm->scanlineH; 
	sub.width = prm->width;
