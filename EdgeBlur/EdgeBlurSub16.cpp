...
typedef struct edgeBlurPrmSub16
{
	A_long		blur;
	A_long		offset;
	A_long		tbl[451];
...
//----------------------------------------------------------------------------
PF_Boolean makeTbl(edgeBlurPrm *prm, edgeBlurPrmSub16 *sub)
{
	PF_InData *in_data;
	in_data = prm->in_data;
...
//----------------------------------------------------------------------------
void horMin(edgeBlurPrmSub16 *sub)
{
	A_long hor =0;
	A_long i;
	A_long x=0, y=0;
	...
	for ( y=0; y<sub->height; y++)
	{
		...
		A_long minIndex = -1;
		A_u_short minValue = PF_MAX_CHAN16;
...
//----------------------------------------------------------------------------
void verMin(edgeBlurPrmSub16 *sub)
{
	A_long hor =0;
	A_long i;
	A_long x=0, y=0;
...
void horBlur(edgeBlurPrmSub16 *sub)
{
	A_long hor =0;
	A_long ir,il,i;
	A_long x=0, y=0;
	...
	for ( y=0; y<sub->height; y++)
	{
		...
		for ( x=0; x<sub->width; x++){
			...
			if (count>0){
				sub->data[hor + x].alpha = RoundShort(sa/count);
			}
		}
	}
...
//----------------------------------------------------------------------------
void verBlur(edgeBlurPrmSub16 *sub)
{
	A_long hor =0;
	A_long ir,il,i;
	A_long x=0, y=0;
	...
	for ( x=0; x<sub->width; x++)
	{
		...
		for ( y=0; y<sub->height; y++){
			...
			if (count>0){
				sub->data[hor].alpha = RoundShort(sa/count);
			}
			hor += sub->widthTrue;
		}
	}
...
PF_Err EdgeBlurSub16(edgeBlurPrm *prm)
{
	...
	edgeBlurPrmSub16 sub;
	sub.data = (PF_Pixel16 *)prm->data;
	sub.scanline =*(A_u_short **)prm->scanlineH; 
	sub.width = prm->width;
